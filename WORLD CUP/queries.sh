...
echo "$($PSQL "SELECT t.name FROM games AS g JOIN teams as t ON t.team_id=g.winner_id WHERE g.year=2018 AND g.round='Final';")"
...
echo "$($PSQL "SELECT distinct t.name FROM games AS g JOIN teams as t ON t.team_id=g.winner_id ORDER BY t.name;")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT g.year, t.name FROM games AS g JOIN teams as t ON t.team_id=g.winner_id WHERE g.round='Final' ORDER BY year;")"
